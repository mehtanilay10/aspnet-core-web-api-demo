...
namespace DemoApp.DataAccess.Models.Paging
{
    public class Paginate<T> : IPaginate<T>
    {
        internal Paginate(IEnumerable<T> source, int index, int size, int from)
        {
            ...
            if (source is IQueryable<T> querable)
            {
                ...
                int skipElements = (Index - From) * Size;
                int takeElements = (Count < Size) ? Count : Size;
                Items = querable.Skip(skipElements).Take(takeElements).ToList();
            }
            else
            {
                Index = index;
                ...
                int skipElements = (Index - From) * Size;
                int takeElements = (Count < Size) ? Count : Size;
                Items = enumerable.Skip(skipElements).Take(takeElements).ToList();
            }
        }
